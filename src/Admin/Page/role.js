...
const RolePage = ({searchTerm,showModal, setShowModal})=>{
    ...
    const handleSaveCreate = ()=>{
        ...
        axios({
            url:`${process.env.REACT_APP_BACKEND_HOST_PROTOCOL}://${process.env.REACT_APP_BACKEND_HOST}/antrian/v1/admin/role`,
            data:{name:nameLoket,uuid:editId},
            method:typeModal=="edit"?"PUT":"POST",
            headers:{Authorization:"Bearer "+token}}
        ).then((res)=>{
            if(res?.data){
                if(typeModal=="edit"){
                    toast.success("Berhasil Edit Role")

                }else{

                    toast.success("Berhasil Create Role")
                }
               setRefresh(aa=>!aa)
               setShowModal(false)
               setNameLoket("")
    ...
    useEffect(()=>{
        if(token){
            ...
            const delayDebounceFn = setTimeout(() => {
                ...
                axios.get(`${process.env.REACT_APP_BACKEND_HOST_PROTOCOL}://${process.env.REACT_APP_BACKEND_HOST}/antrian/v1/admin/role/list?page=${currentPage}&row_perpage=${rowPerPage}&name=${searchTerm}`,{headers:{Authorization:"Bearer "+token}}).then((res)=>{
                    if(res?.data?.data){
    ...
    const handleActiveDeactive =  (id, status)=>{
        // alert(id,status)
        setLoading(true)
        axios.post(`${process.env.REACT_APP_BACKEND_HOST_PROTOCOL}://${process.env.REACT_APP_BACKEND_HOST}/antrian/v1/admin/role/${status=="active"?"deactivate":"activate"}`,{uuid:id},{headers:{Authorization:"Bearer "+token}}).then((res)=>{
            if(res?.data){
               toast.success("Berhasil Update Status Role")
               setRefresh(aa=>!aa)
                ...
                <div style={{display:'flex', justifyContent:'center',alignItems:'center',transition:'0.3s ease',position:'fixed', left:0, right:0, bottom:showModal?0:"100%", top:0, zIndex:6,marginTop:showModal?'0':'-10000px', background:'rgba(232, 235, 237,0.9)'}}>
                    <div  style={{position:'relative',width:'300px', height:'210px',background:'#e8ebed', borderRadius:'25px',    boxShadow: "4px 4px 8px #bdc8d5, -4px -4px 8px #fff"}}>
                            ...
                            <div class="text-input-group" style={{width:'260px'}}>
                                ...
                                required="required"
                                />
                                <label for="firstName">Nama Role</label>
                            </div>
                            </div>
...
<tr>
                                                 ...
                                                 <FiEdit onClick={()=>{setShowModal(true);setTypeModal("edit");setEditId(val?.role_uuid);setNameLoket(val?.name)}} style={{fontSize:'25px', cursor:'pointer',color:'gray'}}  data-tooltip-place="right" data-tooltip-id="my-tooltip"></FiEdit >
  ...
  <div style={{display:'flex', alignItems:'start', justifyContent:'center',flexDirection:'column'}}>
        ...
        <div id="toggle2">
            <input 
            onClick={()=>{handleActiveDeactive(val?.role_uuid,val?.status)}}
            type="checkbox"
            id={val.id}
