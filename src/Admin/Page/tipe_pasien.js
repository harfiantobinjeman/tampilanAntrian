...
const Loket = ({searchTerm,showModal, setShowModal})=>{
    ...
    const handleSaveCreate = ()=>{
        ...
        axios({
            url:`${process.env.REACT_APP_BACKEND_HOST_PROTOCOL}://${process.env.REACT_APP_BACKEND_HOST}/antrian/v1/admin/tipe_pasien`,
            ...
            if(res?.data){
                if(typeModal=="edit"){
                    toast.success("Berhasil Edit Tipe Pasien")

                }else{

                    toast.success("Berhasil Create Tipe Pasien")
                }
               setRefresh(aa=>!aa)
               setShowModal(false)
               setNameLoket("")
    ...
    useEffect(()=>{
        if(token){
            ...
            const delayDebounceFn = setTimeout(() => {
                ...
                axios.get(`${process.env.REACT_APP_BACKEND_HOST_PROTOCOL}://${process.env.REACT_APP_BACKEND_HOST}/antrian/v1/admin/tipe_pasien/list?page=${currentPage}&row_perpage=${rowPerPage}&name=${searchTerm}`,{headers:{Authorization:"Bearer "+token}}).then((res)=>{
                    if(res?.data?.data){
    ...
    const handleActiveDeactive =  (id, status)=>{
        // alert(id,status)
        setLoading(true)
        axios.post(`${process.env.REACT_APP_BACKEND_HOST_PROTOCOL}://${process.env.REACT_APP_BACKEND_HOST}/antrian/v1/admin/tipe_pasien/${status=="active"?"deactivate":"activate"}`,{id:id},{headers:{Authorization:"Bearer "+token}}).then((res)=>{
            if(res?.data){
               toast.success("Berhasil Update Status Tipe Pasien")
               setRefresh(aa=>!aa)
                ...
                <div style={{display:'flex', justifyContent:'center',alignItems:'center',transition:'0.3s ease',position:'fixed', left:0, right:0, bottom:showModal?0:"100%", top:0, zIndex:6,marginTop:showModal?'0':'-10000px', background:'rgba(232, 235, 237,0.9)'}}>
                    <div  style={{position:'relative',width:'300px', height:'210px',background:'#e8ebed', borderRadius:'25px',    boxShadow: "4px 4px 8px #bdc8d5, -4px -4px 8px #fff"}}>
                            <div className="bg-info" style={{justifyContent:'space-between',letterSpacing:'2px',fontSize:'15px',fontWeight:'600',color:'white',boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px", display:'flex',borderTopRightRadius:'25px',borderTopLeftRadius:'25px' ,alignItems:'center',padding:'10px 20px',width:'100%', height:'60px'}} onClick={()=>{setShowModal(aa=>!aa);setNameLoket("");setTypeModal("")}}>
                               {typeModal=="edit"?"EDIT":"CREATE"} TIPE PASIEN
                            ...
                            <div class="text-input-group" style={{width:'260px'}}>
                                ...
                                name="firstName" 
                                id="firstName" 
                                placeholder="Nama Tipe Pasien" 
                                autocomplete="off"
                                required="required"
                                />
                                <label for="firstName">Nama Tipe Pasien</label>
                            </div>
                            </div>
